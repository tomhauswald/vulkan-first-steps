...
public:
    ThirdPersonController(
        Engine& e, Model& targetModel, float yOffset, float distance, 
        float moveSpeed, float sprintSpeed, float rotateSpeed
    ) :
        CameraController(e),
			...
			if(m_moving) {
				m_targetModel.setPosition(
                    ...
                    + glm::normalize(glm::vec3{ dir.x, 0.0f, dir.z }) 
                        * (m_sprinting ? m_sprintSpeed : m_moveSpeed) 
                        * dt
                );
			}
