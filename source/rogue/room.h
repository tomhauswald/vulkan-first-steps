...
public:
	...
	Room(Engine& e, size_t index, glm::vec3 origin, glm::vec3 size, Dir doors) :
		Model(e, e.renderer().createMesh("room" + std::to_string(index)), e.renderer().texture("stonebrick_mossy")),
		m_index(index),
		...
		// ceiling
		addCubeAt({ 0, m_size.y - wallThickness, 0 }, { m_size.x, wallThickness, m_size.z }, { 0, 1, 0 });
        ...
        if(doors & Dir::North) {
            addCubeAt({ 0, wallThickness, m_size.z - wallThickness }, 
                      ...
                      { 0, 0, 1 });
		    addCubeAt({ m_size.x - (m_size.x - doorWidth) / 2.0f, wallThickness, m_size.z - wallThickness }, 
                      { (m_size.x - doorWidth) / 2.0f, doorHeight, wallThickness }, 
                      { 0, 0, 1 });
            addCubeAt({ 0, wallThickness + doorHeight, m_size.z - wallThickness },
        ...
        if(doors & Dir::East) {
            addCubeAt({ m_size.x - wallThickness, wallThickness, wallThickness }, 
                      { wallThickness, doorHeight, (m_size.z - doorWidth) / 2.0f - wallThickness}, 
                      { 1, 0, 0 });
            addCubeAt({ m_size.x - wallThickness, wallThickness, m_size.z - (m_size.z - doorWidth) / 2.0f }, 
                      { wallThickness, doorHeight, (m_size.z - doorWidth) / 2.0f - wallThickness}, 
                      { 1, 0, 0 });
            addCubeAt({ m_size.x - wallThickness, wallThickness + doorHeight, wallThickness }, 
                      { wallThickness, m_size.y - 2 * wallThickness - doorHeight, m_size.z - 2 * wallThickness }, 
                      { 1, 0, 0 });
        } else {
            addCubeAt({ m_size.x - wallThickness, wallThickness, wallThickness }, 
                      { wallThickness, m_size.y - 2 * wallThickness, m_size.z - 2 * wallThickness }, 
                      { 1, 0, 0 });
        }
        ...
        if(doors & Dir::South) {
		    ...
		    addCubeAt({ m_size.x - (m_size.x - doorWidth) / 2.0f, wallThickness, 0 }, 
                      { (m_size.x - doorWidth) / 2.0f, doorHeight, wallThickness }, 
                      { 1, 1, 0 });
            addCubeAt({ 0, wallThickness + doorHeight, 0 }, 
                      { m_size.x, m_size.y - 2 * wallThickness - doorHeight, wallThickness }, 
                      { 1, 1, 0 });
        } else {
            addCubeAt({ 0, wallThickness, 0 }, 
