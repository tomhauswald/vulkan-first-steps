...
public:
	inline Tilemap(glm::u64vec3 size, Texture const& tileset, glm::uvec2 const& srcTileSize, glm::vec2 dstTileSize) :
	...
	inline void clear(size_t layer) {
		fill(layer, 0);
	}
	...
	inline virtual void draw(Renderer& r) override {
		...
		auto maxLoc = glm::u64vec3{
			...
			std::min(m_size.y - 1, minLoc.y + visibleArea.y - 1),
			m_size.z - 1
		};
