...
public:
  inline Camera2d(glm::vec2 viewportSize)
      : m_position{0, 0}, m_viewportSize{std::move(viewportSize)},
        m_viewportHalfSize{m_viewportSize / 2.0f}, m_zoom{1} {}
  ...
  inline bool isWorldRectVisible(glm::vec2 const &pos,
                                 glm::vec2 const &size) const noexcept {
    return (isWorldPointVisible(pos) || isWorldPointVisible(pos + size) ||
  ...
  inline glm::vec4 worldToNdcRect(glm::vec2 const &pos,
                                  glm::vec2 const &size) const noexcept {
    auto vpp = worldToViewportPoint(pos) / m_viewportHalfSize;
    return m_zoom * glm::vec4{vpp.x - 1.0f, 1.0f - vpp.y,
...
public:
  inline Camera3d(float aspectRatio, float fovDegrees)
      : m_position{0, 0, 0}, m_direction{0, 0, 1}, m_aspectRatio{aspectRatio},
        m_fovDegrees{fovDegrees} {

    m_projection = glm::perspectiveLH(glm::radians(m_fovDegrees), m_aspectRatio,
                                      1e-3f, 1e3f);
  }
  ...
  inline glm::mat4 transform() const noexcept {
    return m_projection *
           glm::lookAtLH(m_position, m_position + m_direction, {0, 1, 0});
  }
};
